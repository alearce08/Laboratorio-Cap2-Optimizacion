...
// Ejercicio D: tiempos por region, escritos al terminar la ejecucion.
struct RegionTiming {
  const char *region;
  int iteration;
  double milliseconds;
...
static std::vector<RegionTiming> region_timings;

static void record_timing(const char *region, int iteration,
                          std::chrono::steady_clock::time_point t0,
                          std::chrono::steady_clock::time_point t1) {
  const double ms = std::chrono::duration<double, std::milli>(t1 - t0).count();
  region_timings.push_back({region, iteration, ms});
}

static void print_region_timings() {
  std::ostringstream csv;
  csv << "region,iteration,milliseconds\n";
  csv << std::fixed << std::setprecision(6);
  for (const RegionTiming &timing : region_timings) {
    csv << timing.region << ',' << timing.iteration << ','
        << timing.milliseconds << '\n';
  }
  // stderr permite guardar el CSV separado de la salida original del programa.
  std::cerr << csv.str();
}
...
static IcpResult collimate_icp(const std::vector<Point> &target,
  ...
  // Solo el indice fijo de ICP; los de compare_profiles se incluyen en sus metricas.
  const auto index_t0 = std::chrono::steady_clock::now();
  GridIndex index(target, 90.0);
  record_timing("grid_index_icp", 0, index_t0, std::chrono::steady_clock::now());
  ...
  const auto initial_metrics_t0 = std::chrono::steady_clock::now();
  ProfileMetrics initial_metrics =
      compare_profiles(target, current, match_threshold, missing_distance);
  record_timing("profile_metrics", 0, initial_metrics_t0,
  ...
  for (int iter = 1; iter <= 80; ++iter) {
    ...
    const auto neighbors_t0 = std::chrono::steady_clock::now();
    ...
    record_timing("nearest_neighbors", iter, neighbors_t0,
                  std::chrono::steady_clock::now());
    ...
    record_timing("estimate_transform", iter, estimate_t0,
                  std::chrono::steady_clock::now());
    ...
    const ProfileMetrics metrics =
        compare_profiles(target, current, match_threshold, missing_distance);
    record_timing("profile_metrics", iter, metrics_t0,
                  std::chrono::steady_clock::now());
...
static void export_reconstruction(
  ...
  for (std::size_t i = 0; i < result.snapshots.size(); ++i) {
    ...
    const std::vector<unsigned char> rgb =
        render_motion_frame(target, source, result.snapshots, i, width, height);
    record_timing("render_motion_frame", static_cast<int>(i), render_t0,
                  std::chrono::steady_clock::now());
    write_ppm(output_dir / name.str(), rgb, width, height);
  }
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    record_timing("generate_target", 0, target_t0,
                  std::chrono::steady_clock::now());

    const auto source_t0 = std::chrono::steady_clock::now();
    ...
    record_timing("prepare_source", 0, source_t0,
                  std::chrono::steady_clock::now());
    ...
    if (export_outputs) {
      ...
      record_timing("export_reconstruction", 0, export_t0,
                    std::chrono::steady_clock::now());
    }
