...
static ProfileMetrics compare_profiles(const std::vector<Point> &target,
                                       ...
                                       double missing_distance) {
  GridIndex target_index(target, GRID_CELL_SIZE); // Cambio de 90.0 a GRID_CELL_SIZE
  GridIndex source_index(source, GRID_CELL_SIZE);
...
static IcpResult collimate_icp(const std::vector<Point> &target,
  ...
  GridIndex index(target, GRID_CELL_SIZE); // Cambio de 90.0 a GRID_CELL_SIZE
  auto t_grid_end = std::chrono::steady_clock::now();

  double grid_construction_ms =
      std::chrono::duration<double, std::milli>(t_grid_end - t_grid_start).count();
  
  std::cout << "instrumentation,grid_construction," << grid_construction_ms << "\n";
  ...
  for (int iter = 1; iter <= 80; ++iter) {
    ...
    // Instrumentación: búsqueda de vecinos
    auto t_nearest_start = std::chrono::steady_clock::now();
    ...
    auto t_nearest_end = std::chrono::steady_clock::now();

    double nearest_neighbors_ms =
        std::chrono::duration<double, std::milli>(
            t_nearest_end - t_nearest_start).count();

    std::cout << "instrumentation,nearest_neighbors,"
              << iter << ","
              << nearest_neighbors_ms << "\n";
    ...
    // Instrumentación: estimación de transformación rígida
    auto t_transform_start = std::chrono::steady_clock::now();

    const Transform2D delta = estimate_rigid_transform(matches);

    auto t_transform_end = std::chrono::steady_clock::now();

    double rigid_transform_ms =
        std::chrono::duration<double, std::milli>(
            t_transform_end - t_transform_start).count();

    std::cout << "instrumentation,rigid_transform,"
              << iter << ","
              << rigid_transform_ms << "\n";
    ...
    // Instrumentación: métricas de comparación
    auto t_metrics_start = std::chrono::steady_clock::now();

    const ProfileMetrics metrics =
        compare_profiles(target, current, match_threshold, missing_distance);

    auto t_metrics_end = std::chrono::steady_clock::now();

    double profile_metrics_ms =
        std::chrono::duration<double, std::milli>(
            t_metrics_end - t_metrics_start).count();

    std::cout << "instrumentation,profile_metrics,"
              << iter << ","
              << profile_metrics_ms << "\n";
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    // Instrumentación: generación del perfil objetivo
    auto t_target_start = std::chrono::steady_clock::now();

    std::vector<Point> target =
        generate_h_rail_cloud(points_per_cloud, 7);

    auto t_target_end = std::chrono::steady_clock::now();

    double target_generation_ms =
        std::chrono::duration<double, std::milli>(
            t_target_end - t_target_start).count();

    std::cout << "instrumentation,target_generation,"
    ...
    // Fin de medición
    auto t_source_end = std::chrono::steady_clock::now();

    double source_preparation_ms =
        std::chrono::duration<double, std::milli>(
            t_source_end - t_source_start).count();

    std::cout << "instrumentation,source_preparation,"
    ...
    if (export_outputs) {
    // Instrumentación: exportación de CSV y frames PPM
    ...
    double file_export_ms =
        std::chrono::duration<double, std::milli>(
            t_export_end - t_export_start).count();

    std::cout << "instrumentation,file_export,"
              << file_export_ms << "\n";
  }
   ...
   // if (viewer) {
    ...
    // Instrumentación: renderizado y envío de frames a GStreamer
    auto t_gstreamer_start = std::chrono::steady_clock::now();

    show_with_gstreamer(target, source, result.snapshots);

    auto t_gstreamer_end = std::chrono::steady_clock::now();

    double gstreamer_ms =
        std::chrono::duration<double, std::milli>(
            t_gstreamer_end - t_gstreamer_start).count();

    std::cout << "instrumentation,gstreamer,"
              << gstreamer_ms << "\n";
  }
